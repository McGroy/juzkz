...
## 14. Результаты проверки проекта и обязательные правки

Проект проверен двумя независимыми верификаторами (полные разборы — `../analysis/verification/verify-coverage-consistency.md` и `../analysis/verification/verify-fidelity.md`). **Подтверждено без расхождений**: все мутации и действия UI фронта имеют эндпоинт и сервис с совпадающими параметрами; все поля `types.ts` отображены на модели; 11 статусов, роли, 105 видов документов и их флаги совпадают; таблицы переходов соответствуют автоматам фронта; форматы номеров, ~110 кодов действий, 45+ статусов, полный контракт ЕРСОП, параметры Keycloak, таблицы `surfk.*`, таймеры и паттерны prof (базовые классы, сервисы, `APIView` на действие, RBAC, конверт ошибок, seed, compose) выверены по первоисточникам.

Ниже — замечания, которые **принимаются как правки к разделам выше** (при реализации руководствоваться этой таблицей; текст разделов 1–13 не переписывался ради экономии).

### 14.1. Блокирующее и существенные (исправить до итерации I4)

| id | Раздел | Что не так | Правка |
|---|---|---|---|
| K-01 | §5.3, §6, §7.3 | `ActionSpec` по умолчанию требует домен `evga_cases` (EDIT/APPROVE/SIGN), а роли `evga-quality`, `evga-qc-head`, `evga-appeal-*` и комиссия имеют в `evga_cases` только `VIEW` → `HasDomainLevel` даст 403 на создание/сохранение/отправку/согласование заключений КК и результатов возражений | В `DocumentActionView`/`DocumentViewSet` домен вычисляется по документу: `domain_for(document)` → `quality*` → `evga_quality`, `objection-result`/`objections` → `evga_appeals`, иначе `evga_cases`; добавить матричный тест «роль × действие × вид документа» |
| C-01 | §6.2, §10.1 | DTO версии не содержит `reviewers`, `reviewerIndex`, `approver`, которые читают `DocumentActions.tsx`, `ReferencePrintForm.tsx`, `DocumentWorkspace.tsx` | Сериализатор версии отдаёт производные поля из маршрута: `reviewers` (id согласующих по порядку), `reviewerIndex` (число `approved`), `approver` (id signer); описать в `evgaAdapter` |
| C-09 | §4.6, §5.2, §6.1 | `AuditCase.subject` NOT NULL, но встречное дело с физлицом (`CounterChecks.tsx`) объекта-ЮЛ не имеет | `subject = FK(null=True)` + `CheckConstraint(subject IS NOT NULL OR person_iin IS NOT NULL)`; либо создавать `Subject(subject_kind=NATURAL)` для ФЛ |
| C-10 | §5.4.3 | Подписи РГ задания: проект требует «все не-лидеры + подпись лидера при approve», фронт (`preparationWorkflow.ts`) требует GROUP-подпись каждого участника, включая лидера, затем отдельное утверждение лидером | Guard `approve-group`: у каждого `VersionParticipant` есть `DocumentSignature(kind=GROUP)`; действие `approve` подпись не создаёт |
| K-02 | §5.3 | `apply_action` читает версию до вызова сервиса; для `revise`, `submit` из `RETURNED`, `assign_quality_expert` сервис создаёт новую версию, а `row_version`/`commit_version`/`record_event`/ответ относятся к замороженной | Сервис возвращает актуальную версию; `apply_action` перечитывает `document.active_version` после сервиса |
| K-03 | §5.3, §7.3 | `actor_role` — одна роль по приоритету; пользователь с несколькими назначениями (аудитор + согласующий) не пройдёт проверку `spec.roles` | `actor_roles(user) -> set`, проверка пересечения; `available_actions` — объединение по всем ролям |
| K-04 | §4.2 vs §2.1 | `core.AuditEvent.case` — FK на `evga_cases.AuditCase`: общее приложение зависит от доменного, нарушает правило аддитивности и слияние в монорепо | `AuditEvent.case_id = UUIDField(null=True, db_index=True)` без FK (или GenericFK); связь резолвится сервисом |
| K-05 | §4.1, §5.4.1, §6.11 | Ревизия «копирует attachments», но `Attachment` привязан к одной версии: ссылки новой версии указывают на файлы старой, `DELETE` в новом черновике удалит файл замороженной версии | `attachments.clone_for_version()`: клон строк `Attachment` с тем же объектом в MinIO, переписать `id` в `values`/`slot`; объект хранилища удалять только если на него нет других ссылок |
| F-07 | §3.2, §4.3, §7.1 | `User.iin unique` + провижининг prof по `(issuer, sub)`: второй `sub` с тем же ИИН (dev/test realm) даст `IntegrityError` | В `provision_user_from_claims` — поиск существующего пользователя по `iin` и привязка новой `FederatedIdentity` к нему (аналог legacy `ON CONFLICT (iin)`); описать явно, «6 строк» недостаточно |
| F-01, F-03, F-04 | §2.3, §3.2, §5.2, §6.9, §10.3 | Названы «копиями из prof» сущности, которых в prof нет: `deploy/nginx.conf` (в prof конфиг nginx — текст в `deploy/README.md`, лимит 25m), `StaleVersionError`, `attach_version_file`/`delete_version_attachment` (в prof `attach_file`/`document_attachments`, удаления нет), `record_acknowledgement(..., by=)` (в prof `recorded_by=`, `acknowledged_on` обязателен), `_assert_subject_owns(user, case)` (в prof `subject_id`), `CabinetCaseViewSet`, `requirements/production.txt` (есть только `base`/`local`), `PrescriptionItemFilter` (в `execution/api/filters.py`) | Пометить как «новое / адаптация»; при переносе использовать prof-сигнатуры; `deploy/nginx.conf` — новый файл ЭВГА, лимит 50m — изменение ЭВГА |
| L-01 | §5.3, §5.6 | Коды `apply_amendment`, `new_version` названы legacy-кодами, но в n8n/BPMN их нет (версии в legacy — флаг `shouldCreateVersion`) | `legacy_codes=()`; убрать из §5.6; тест-инвариант не должен их «подтверждать» |
| L-02 | §4.16 | В `LEGACY_STATUS_MAP` нет статуса `created` (его пишут `Create Case v4/v3`, `Create Case Document`) | `created → DRAFT`; тест полноты расширить статусами из n8n, не только из BPMN |

### 14.2. Мелкие уточнения (учесть при реализации соответствующего раздела)

- **Модель и сериализация**: добавить `description` в `Attachment`/`AttachmentRef` (печать использует) (C-02); отдавать хеш `contentSnapshot` и сравнивать хеши в `amendments.ts`, либо перевести `AmendmentsPanel` на `available_actions` (C-03); маппинг 7 статусов регистрации на 3 строки фронта (`SENT/PENDING/ERROR → «Отправлена»`, `RETURNED/REJECTED → «Возвращена»`) (C-04); `ConfirmationStatus` отдавать в нижнем регистре (C-05); `form-links` — ключи как во фронте (`violationId, riskId, questionId, resultId, field`), принимать несохранённые `values` в теле (C-06); правило стадии РД — `financial && number > 5` (C-07); уточнить семейства для `claim-decisions` (DIRECT) и `request/counter-request` (C-08); `KvgaConfirmation`/`DocumentDelivery` — `update_or_create` при повторной отправке (C-18); объявить `AcknowledgementChannel`, `ErsopExchangeDirection`, `RowProjection`, `FK(User)` (K-10); `evga_case_documents.activated_at` не существует, есть `approvals.status='activated'` (L-04); реальные `signature_type` legacy: `approve, confirm, sign, kvga_confirm, kvga_reject, oa_acknowledge, work_group_*, invited_specialist_*` (L-05); `M21-AKO` в legacy — тип документа, в новой системе — не документ (L-18); `AuditQuestion.reg_number` в legacy нет (L-24).
- **Переходы и сервисы**: `group-request` без `main_preparation_block` (C-11); `activate` без `creation_block` (C-12); `assign_quality_expert` — без guard «эксперт не в участниках», но с эффектом `owner := новый эксперт` для неактивной версии (C-13); `send_to_quality` — guard «не (RETURNED и есть маршрут)» (C-14); `ir-refuse`/`ir-resend` требуют текст, `resend` — `deadline` (C-15); текст 409 для `approve/return/reject` — «…Решение недоступно.» (C-16); `ersop-send` только из `ACTIVE` (K-06); `ersop-register` (стенд) — домен `evga_cases` + флаг стенда, а не `evga_admin` (K-07); циклические импорты `actions ↔ evga_ersop/evga_cases` — регистрация `ActionSpec` из приложений-владельцев (K-11); `ApprovalWorkflowError(STALE_VERSION)` наследовать от `StaleVersionError` → 409 (K-14); `comment_required` с кодом и текстом по действию («Укажите причину отклонения/возврата») (K-15); `Transition` с полем `decision` для действий с разным `to_status` по payload (`kvga-decide`, `registry-decide`, `ersop-check-status`) (K-17); `create_signer_only_route` вместо `create_route(stages=[])` (K-18); префикс «`<docName> · vN: …`» в истории дела (K-19); `allowed_when_closed` для `ersop-check-status`/`ersop-register`/`validate` (K-20); `oa_respond` в legacy нет (L-03); `M44-VK-UK`/`M47-VK-TU` в legacy регистрируются без ЕРСОП (`register → registered`) — регистрация встречных документов через ЕРСОП — решение по фронту (L-06).
- **Роли и Keycloak**: `area: "appeal"` нужен всем `evga-appeal-*` (C-20); `role_id 4/28` = руководители КК, `5/30` = эксперты КК, `7` = `appeal_head`, `8` = `appeal_expert`; привязка к `kvga_kk_*` не подтверждена (L-17); claim `departments[]` — только портальный, для ЭВГА не используется (L-16); 403 у DRF — `permission_denied`, чужой скоуп в prof — 403, `is_active=False` → 401 (F-06).
- **Legacy-справочники**: `legacy_id` подтверждены только для 1 (ИПИ) и 6 (Дело КК), остальные — номера BPMN-процессов (L-07); код внепланового `'2'` не подтверждён (legacy: `PLANNED_CODES ['1','scheduled']`, `inspection_type_id === 6` — внеплановый) (L-10); `audit_type_id=8 → M6-PA-F` подтверждён, 7 — вывод по ELSE (L-11); колонки `evga_doc_info_request`: `doc_status`, `item_status`, `has_new_changes`, `info_request_id` (L-12); справочных воркфлоу `EVGA: * - Get` — 30, не 20 (L-13); `statusName` в ЕРСОП нет — `statusMessages` (L-14); legacy опрашивает ЕРСОП по `requestId = case_document_id`, `ersop_error` возвращает документ в `draft` (L-15); автозакрытия дела при `kvga_return` в legacy не было — снять риск (L-19); `rnn` — фильтр фронта, не legacy (L-20); `registration_number` legacy выдаёт при создании, суффикс `_1` только при `supports_version` (L-22); источники полей ЕРСОП для миграции: `assigning_authority_id → organCode`, `registration_authority_id → oraganCodeKPSSU`, `ersop_subject_id/ersop_object_id`, `uo_check_number/date`, `audit_period_*`, `coverage_period_*` (L-23); `awaiting_objections/reviewing_objections/control/audit_materials_implementation` — все четыре статусы дела (L-25); реальные колонки `evga_status_transitions`: `document_type_id, from_status_id, action_code, action_name_ru/kz, required_role_code, requires_signature, requires_comment, is_automatic` (без `workflow_code/assignment_type/to_status_id`) (L-26); `sent_to_qc` — статус документа `qc_stage3` (L-08); коды фаз дела `preparation/ready_qc1/qc1/…` — новый `CasePhase`, а не коды `CaseProcessV1` (L-09); BPMN `send_notification` бьёт в портальный `/api/notifications` (L-21).
- **Уведомления**: описать `recipients_for_case_event` (назначение эксперта КК/апелляции, соавтор) по `appeals.ts`, `qualityAssignment.ts`, `caseAccess.ts` (C-19).
- **План и цифры**: приложений `evga_*` — семь плюс `notifications` (восемь новых); ролей — 15 `evga-*` + `observer` (K-08); порядок `INSTALLED_APPS` и порядок реализации унифицировать (K-09); `deadlines.py` (рабочие дни) нужен уже в I6/I7 — календарь и рабочие дни перенести в I1 (K-12); оценки I3 (формы) ≈14 и серверный PDF выделить отдельной итерацией ≥10 чел.-дн. — **итого ≈105–110 чел.-дн. бэкенда** вместо ≈94 (K-13); счётчики тестов фронта — `bpmn-main-workflow` 17, `shared-approval-workflow` 26, `npa-compliance` 17, всего ≈199 сценариев бизнес-правил (C-17); `scheduler` на образе prof повторит `entrypoint.sh` — переопределить `entrypoint` и `depends_on: backend (healthy)` (F-08); зачистить черновую фразу про `EvgaMeSerializer` в §3.2 (F-09); scope последовательности `evga-case` — зафиксировать годовой счётчик (legacy) или сквозной (фронт) (K-16).
- **Приложение «не подтверждено»** дополнить: `legacy_id` (L-07), код внепланового/id 7 (L-10, L-11), `departments[]` (L-16), `reg_number` (L-24), `requestId` (L-15), формат `evga_status_transitions` (L-26); исправить п. 7 (L-12) и п. 6 (L-13).

...
| `analysis/verification/verify-coverage-consistency.md`, `verify-fidelity.md` | Разборы двух независимых проверок итогового проекта (покрытие фронта и согласованность; верность prof и достоверность ссылок на старую систему); принятые правки — `design/design-final.md` §14 |
...
8. **RBAC prof без изменений кода**: домены `evga_cases / evga_quality / evga_registry / evga_appeals / evga_execution / evga_cabinet`, уровень `CONFIRM`; 16 ролей (15 `evga-*`, включая `evga-admin`, плюс `observer`) с таблицей соответствия Keycloak-ролям старой системы (важно: старый `approver` = фронтовый `reviewer`, старый `confirmer` = фронтовый `approver`); адресные назначения (эксперт КК, подтверждающий КВГА/реестра, соавтор) — таблицы + проверки в сервисах.
...
12. **Демо на сервере как можно раньше**: каркас → справочники и объекты → дела → документы/версии/черновики → маршрут/инбокс/уведомления (Демо-1 после ≈36 чел.-дн. бэкенда, при двух разработчиках ≈ 4 недели), далее этапы процесса по порядку с демо после каждого; полный объём ≈ 105–110 чел.-дн. бэкенда (базовая оценка ≈ 94 скорректирована после проверки: формы и серверный PDF) + ≈ 36 чел.-дн. фронта (§6).
...
После проверки проекта (`design/design-final.md` §14, K-12/K-13) оценка уточнена: I3 (формы, схемы, валидация ≈2 200 строк правил фронта) ≈ 14 вместо 10; серверный PDF (порт `ReferencePrintForm.tsx` 1 976 строк + `QualityPrintForm` + `pdfExport`) выделить отдельной итерацией I12 ≥ 10; календарь и расчёт рабочих дней перенести из I8 в I1 (нужны в I6–I7 для сроков возражений и требований). Итого **≈105–110 чел.-дн. бэкенда**, Демо-1 по-прежнему после I4 (≈40).
...
| Роли | 14 ролей (A) / 22 роли из Keycloak (B) / 15 без префикса (C) | **16 ролей: 15 `evga-*` (включая `evga-admin`) + `observer`**, маппинг с инверсией `approver ↔ reviewer`, `confirmer ↔ approver` | покрывает фронт и старую систему; синхронизация за флагом |
...
| План | 12 итераций ≈92 (A) / 9 ≈93 (B) / 6 ≈40 + техдолг (C) | **I0–I11 ≈94 (после проверки ≈105–110) + ≈36 фронт, Демо-1 после I4** | оценка C занижена из-за отложенных проекций и тестов |

**Проверка проекта.** Итоговый проект прошёл две независимые проверки: покрытие фронта и внутренняя согласованность (39 замечаний: 1 блокирующее, 7 существенных, 31 мелкое; покрытие всех мутаций и полей фронта подтверждено) и верность prof и достоверность ссылок на старую систему (35 замечаний: 5 существенных, 30 мелких; форматы номеров, коды действий и статусов, контракт ЕРСОП, параметры Keycloak и паттерны prof подтверждены). Все замечания с принятыми правками сведены в `design/design-final.md` §14, полные разборы — `analysis/verification/`. Главное, что нужно учесть до итерации I4: домен прав вычисляется по виду документа (иначе роли КК и апелляции получат 403), DTO версии должен отдавать `reviewers/reviewerIndex/approver`, встречное дело с физлицом допускает `subject = null`, подписи РГ задания ставят все участники включая лидера, версия перечитывается после сервиса ревизии, у пользователя может быть несколько ролей, `AuditEvent.case_id` — без FK на доменное приложение, вложения клонируются при ревизии, провижининг Keycloak сливает пользователей по ИИН; ряд сущностей, названных «копиями prof», в prof отсутствует и помечен как новые.

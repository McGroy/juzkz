...
(function () {
  ...
  /* ------------------------------------------------------------------
     ...
     Запускается заново после каждой смены языка: переводы строк с
     разметкой (data-i18n-html) перезаписывают innerHTML целиком, а вместе
     с ним — уже подставленные телефон, БИН и адрес. Так бывает, например,
     в первом абзаце политики обработки персональных данных.
     ------------------------------------------------------------------ */
  ...
  function initContacts() {
    fillContacts();
    document.addEventListener("i18n:change", fillContacts);
  }
